...
# wild, and podman ships a compose of its own, so find whichever this machine has
# rather than guessing. Every shape is tried because a project that ships no
# quadlets is driven through compose under podman too.
compose() {
    if docker compose version >/dev/null 2>&1; then
        ...
        podman compose -f "${DEV_COMPOSE_FILE}" "$@"
    elif command -v podman-compose >/dev/null 2>&1; then
        podman-compose -f "${DEV_COMPOSE_FILE}" "$@"
    else
        echo "No Compose found. Install the Docker Compose plugin (docker compose)," >&2
        echo "the standalone docker-compose, or podman-compose, then retry." >&2
        exit 1
    fi
}
...
# Whether this project defines its stack as quadlets at all. A project that does
# not ships a compose file and nothing else, and an unmatched glob would otherwise
# reach systemctl as the literal "*.container.service".
has_quadlets() {
    compgen -G "${QUADLET_DIR}/*.container.in" >/dev/null 2>&1
}
...
task_env() {
    ...
    # Which driver runs the stack is decided by what the project ships and not by
    # the runtime alone: quadlets buy systemd's ordering and readiness, but they
    # only exist if the project wrote them. Under podman with no units, compose is
    # the driver — which is what a generated project starts life with.
    local runtime
    runtime=$(require_container_runtime)
    if [[ "${runtime}" == "podman" ]] && has_quadlets; then
        quadlet_env "${action}"
    else
        compose_env "${action}"
    fi
